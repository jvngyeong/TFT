...
CREATE TABLE PAYMENT (
	...
	RESULTMESSAGE VARCHAR(2000) NULL,
...
CREATE TABLE INQUIRE (
	...
	MEMBER_NUM VARCHAR(50) NOT NULL,
	goods_num VARCHAR(50) NOT NULL,
	INQUIRE_SUNBJECT VARCHAR(255) NULL,
...
-- 퇴사자
ALTER TABLE RETIRED
	ADD CONSTRAINT PK_RETIRED -- 퇴사자 기본키
	PRIMARY KEY (
	emp_num -- 직원번호
	);
...
-- 문의
ALTER TABLE INQUIRE
	ADD CONSTRAINT PK_INQUIRE -- 문의 기본키
	PRIMARY KEY (
	INQUIRE_NUM, -- 문의번호
	MEMBER_NUM,  -- 회원번호
	goods_num    -- 상품번호
...
-- 상품
ALTER TABLE GOODS
	ADD CONSTRAINT FK_EMPLOYEES_TO_GOODS -- 직원 -> 상품
	FOREIGN KEY (
	emp_num -- 등록직원번호
	)
...
-- 상품
ALTER TABLE GOODS
	ADD CONSTRAINT FK_EMPLOYEES_TO_GOODS2 -- 직원 -> 상품2
	FOREIGN KEY (
	update_emp_num -- 수정직원번호
	)
	REFERENCES EMPLOYEES ( -- 직원
	emp_num   -- 직원번호
	) on delete set null;
...
-- 입고
ALTER TABLE GOODS_IPGO
	ADD CONSTRAINT FK_EMPLOYEES_TO_GOODS_IPGO -- 직원 -> 입고
	FOREIGN KEY (
	emp_num -- 직원번호
	)
	REFERENCES EMPLOYEES ( -- 직원
...
-- 입고
ALTER TABLE GOODS_IPGO
	ADD CONSTRAINT FK_GOODS_TO_GOODS_IPGO -- 상품 -> 입고
	FOREIGN KEY (
	goods_num -- 상품번호
	)
	REFERENCES GOODS ( -- 상품
...
-- 주문상품
ALTER TABLE PURCHASE_LIST
	ADD CONSTRAINT FK_PURCHASE_TO_PURCHASE_LIST -- 주문 -> 주문상품
	FOREIGN KEY (
	PURCHASE_NUM -- 주문번호
	)
...
-- 후기
ALTER TABLE REVIEWS
	ADD CONSTRAINT FK_PURCHASE_LIST_TO_REVIEWS -- 주문상품 -> 후기
	FOREIGN KEY (
	goods_num,    -- 상품번호
	PURCHASE_NUM  -- 주문번호
	)
	REFERENCES PURCHASE_LIST ( -- 주문상품
	goods_num,    -- 상품번호
	PURCHASE_NUM  -- 주문번호
	) on delete cascade;
